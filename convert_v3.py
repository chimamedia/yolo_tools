...
def create_config(data_dir, train_ratio):
    ...
    # create yolov3 cfg file
    base_config3 = open("yolov3.cfg", "r")
    # new_config = open("{0}/darknet_data/config/learning.cfg".format(data_dir), "w")
    new_config3 = open("{0}/config/learning.cfg".format(data_dir), "w")

    for line in base_config3.readlines():
        # if line.find("$FILTERS_NUM") != -1:
        if line.find("filters=255") != -1:
            filter_num = 3 * (len(classes) + 4 + 1)
            new_config3.write("filters={0}\n".format(filter_num))
        elif line.find("classes") != -1:
            new_config3.write("classes={0}\n".format(len(classes)))
        else:
            new_config3.write(line)
    base_config3.close()
    new_config3.close()
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print ("python convert_v3 [input_folder] [output_folder] [learing_ratio] [backup_folder]\n\n")
        exit(-1)
    data_dir = sys.argv[1]
