...
def create_config(data_dir, train_ratio):
    ...
    for i in classes:
        ...
        list_file = open("{0}/config_voc2/{1}_list.txt".format(data_dir, i), 'r')
    ...
    train_list_file = open("{0}/config_voc2/train.txt".format(data_dir), "w")
    for file_name in train_list:
        train_list_file.write(file_name)
    train_list_file.close()
    # valid_list_file = open("{0}/darknet_data/config/valid.txt".format(data_dir), "w")
    valid_list_file = open("{0}/config_voc2/valid.txt".format(data_dir), "w")
    ...
    data_config = open("{0}/config_voc2/learning.data".format(data_dir), "w")
    data_config.write("classes = {0}\n".format(len(classes)))
    data_config.write("train = {0}/config_voc2/train.txt\n".format(data_dir))
    data_config.write("valid = {0}/config_voc2/valid.txt\n".format(data_dir))
    data_config.write("names = {0}/config_voc2/learning.names\n".format(data_dir))
    data_config.write("backup = backup\n")
    data_config.close()
    
    names_config = open("{0}/config_voc2/learning.names".format(data_dir), "w")
    for i in classes:
        names_config.write("{0}\n".format(i))
    names_config.close()

    base_config = open("yolo-voc.2.0.cfg", "r")
    # new_config = open("{0}/darknet_data/config/learning.cfg".format(data_dir), "w")
    new_config = open("{0}/config_voc2/learning.cfg".format(data_dir), "w")
...
def main(data_dir, obj_dir):
    ...
    for cls in classes:
        ...
        # configフォルダを作成
        confDir = "{0}/config_voc2".format(data_dir)
        if not os.path.exists(confDir):
            os.makedirs(confDir)
        
        # outputファイルのpathが列挙されるリストファイル
        list_file = open('%s/%s/config_voc2/%s_list.txt'%(wd, data_dir, cls), 'w')
...
if __name__ == '__main__':
    ...
    #不要なリストを削除する
    for i in classes:
        subprocess.call("del {0}\\config_voc2\\{1}_list.txt".format(data_dir, i), shell=True)